...
class MDI_DB_cubrid_forge extends CI_DB_cubrid_forge {
    ...
    public function is_table_exists($table) {
        $query = $this->db->query(
            "SELECT * FROM db_class WHERE class_name = ".
    ...
    public function is_field_exists($table, $field) {
        $query = $this->db->query(
            "SELECT * FROM db_attribute WHERE class_name = ".
            $this->db->escape($table).
            " AND attr_name = ".
...
/* Location: /third_party/mdi/system/database/drivers/cubrid/cubrid_forge.php */